...
/* One hue, several depths: the split reads as one system, not a row of flags. Goals share a shade. */
const COLORS: Record<string, string> = {
  buffer: "bg-accent-strong",
  investing: "bg-accent",
  goal: "bg-accent/60",
  unassigned: "bg-[var(--accent-wash)]",
  free: "bg-[var(--accent-wash)]",
  spendable: "bg-positive/70",
};
const TEXT: Record<string, string> = { buffer: "text-ink", investing: "text-ink", free: "text-ink" };
const color = (id: string) => COLORS[id] ?? (id.startsWith("goal_") ? COLORS.goal : "bg-accent");
  ...
  return (
    <div className="grid gap-2.5">
      <div className="flex h-3 overflow-hidden rounded-full bg-[var(--sidebar-active)]">
        ...
        {total > 0n
          ? visible.map((s) => <div key={s.id} className={color(s.id)} style={{ width: `${Number((s.value * 1000n) / total) / 10}%` }} title={`${s.name} ${formatCents(s.value)}`} />)
          : null}
        </div>
      </div>
      {legend ? (
        <div className="flex flex-wrap gap-x-4 gap-y-1 whitespace-nowrap text-[12px] text-ink2">
          {segments.map((s) => (
            <span key={s.id} className="inline-flex items-center gap-1.5">
              <span className={`inline-block h-2 w-2 rounded-[2px] ${color(s.id)}`} />
